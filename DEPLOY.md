...
| `apps/web/.env.local` | `API_URL=http://127.0.0.1:4000`, `NEXT_PUBLIC_SITE_URL=https://happenmcr.com`, **`DATABASE_URL`** (same DB as API — Auth.js), **`AUTH_SECRET`**, **`AUTH_URL=https://happenmcr.com`**, **`AUTH_GOOGLE_ID` / `AUTH_GOOGLE_SECRET`**, **`AUTH_FACEBOOK_ID` / `AUTH_FACEBOOK_SECRET`**, **`MAPBOX_ACCESS_TOKEN`**, optional `NEXT_PUBLIC_GA_MEASUREMENT_ID` (GA4 `G-…`), optional `NEXT_PUBLIC_GTM_ID`, optional `NEXT_PUBLIC_ADSENSE_CLIENT` |
